...
function Update () {
	...
	if(Vector3.Distance(player.transform.position, transform.position) < 5 || Vector3.Distance(ai.transform.position, transform.position) < 5) { 
		if(chosenCharacter.renderer.material.color == Color.red && chosenShapeScript.polySize == 3) {
			
			Debug.Log("Puzzle1 Unlocked");
			bridgeScriptLevel3.showSolution1 = true;
		}
		else {
			bridgeScriptLevel3.showSolution1 = false;
		}
	}
	else {
		bridgeScriptLevel3.showSolution1 = false ;
